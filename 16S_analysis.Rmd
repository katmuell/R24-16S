...
Calc.Top20.by.Patient <- function(subset, graphtitle) {
  top_otus <- names(sort(taxa_sums(subset), decreasing = TRUE))[1:20]
  subset_top20 <- prune_taxa(top_otus, subset)
  barplot <- plot_bar(subset_top20, x = "Visit_number", fill = "Phylum") +
    geom_bar(stat = "identity", position = "stack", size = 0) +
    facet_wrap(~ Patient)
    labs(title = graphtitle)
...
begin_end_plot <- Calc.Top20.by.Patient(begin_end, "Start and Endpoint by Patient")
```
