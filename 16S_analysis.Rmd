...
Take only baseline visits
```{r}
base_whole_plot <- plot_bar(baseline_whole, x = "Patient", fill = "Phylum") +
  geom_bar(stat = "identity", position = "stack", size = 0) +
  labs(title = "Baseline")
base_whole_plot
```
```{r}
test_df <- psmelt(baseline_whole)

test_verruco <- test_df %>%
  filter(Phylum == "D_1__Verrucomicrobia") %>%
  group_by(Patient) %>%
  summarise(verruco_abundance = sum(Abundance)) %>%
  as.data.frame()
test_verruco <- test_verruco[order(-test_verruco$verruco_abundance),]
test_df$Patient <- factor(test_df$Patient, levels = test_verruco$Patient)

getPalette = colorRampPalette(brewer.pal(9, "Set1"))
phyla_palette = getPalette(25)
test_plot <- ggplot(test_df, aes(x = Patient, y = Abundance, factor(Phylum), fill = factor(Phylum))) +
