...
group_abundances <- rbind(
  data.frame(group = "baseline", abundance = sample_data(obese.baseline.ps)$Akk_abund),
  data.frame(group = "endpoint", abundance = sample_data(obese.endpoint.ps)$Akk_abund),
  data.frame(group = "healthy", abundance = sample_data(healthy.ps.rel)$Akk_abund)
)
...
Baseline
```{r}
baseline_distribution <- group_abundances %>%
  filter(group == "baseline") %>%
  ggplot(aes(x = abundance)) +
  geom_histogram()

logbaseline_distribution <- group_abundances %>%
  filter(group == "baseline") %>%
...
Endpoint
```{r}
endpoint_distribution <- group_abundances %>%
  filter(group == "endpoint") %>%
  ggplot(aes(x = abundance)) +
  geom_histogram()

logendpoint_distribution <- group_abundances %>%
  filter(group == "endpoint") %>%
  ggplot(aes(x = log_abundance)) +
  geom_histogram()
```
...
Healthy
```{r}
healthy_distribution <- group_abundances %>%
  filter(group == "healthy") %>%
  ggplot(aes(x = abundance)) +
  geom_histogram()

loghealthy_distribution <- group_abundances %>%
  filter(group == "healthy") %>%
  ggplot(aes(x = log_abundance)) +
  geom_histogram()
```
